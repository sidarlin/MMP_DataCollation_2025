...
# master_spi_species$Species_scientific <- master_spi_species$Species
...
# master_species_join <- master_species_join %>%
#   mutate(across(everything(), ~ iconv(., from = "", to = "UTF-8", sub = " ")))  # Replace invalid characters
# Encoding(master_species_join$Species_scientific)  # Check encoding
# unique(master_species_join$Species_scientific)    # Look for strange characters
# 
# write.csv(master_species_join, "temp_data.csv", row.names = FALSE, fileEncoding = "UTF-8")
# master_species_join <- read.csv("temp_data.csv", stringsAsFactors = FALSE, encoding = "UTF-8")
...
species_map_data <- master_species_join %>%
  ...
  select(-c(Species.x, Species.y, Study_Area, Description, 
...
Let's add in the incidental data and the DNA data to map presence only 
...
DNA_data <- read.csv(file="DNA_data_MDB_03-06.csv", header=T)
```


```{r}
## Add Incidentals
setwd("C:/LocalR/Merge_datacollation_2025/Incidental_observations")

inci_data <- read.csv(file="Incidental_data_MDB_09_04_2024.csv", header=T)
```
