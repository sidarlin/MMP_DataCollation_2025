...
# DNA_data$Date_Time <- as.Date(DNA_data$End_Deployment_date, format = "%Y-%m-%d %H:%M:%s")
...
Load other incidentals clipped to BC Boundary
...
table(incidentals_clip$Species_common_name)
```




Attach incidentals to the Camera/DNA with metadata sheet
...
all_sources_species <- bind_rows(cams_dna_sp_meta, inci_spi_data, incidentals_clip)
...
### Clean up some names that got missed

table(all_sources_species$Species_common_name)

all_sources_species$Species_common_name <- recode(all_sources_species$Species_common_name,
...
all_sources_species$Species_scientific <- recode(all_sources_species$Species_scientific,
...
Let's map all Marten by data sharing permissions
```{r}
all_sources_species <- all_sources_species %>%
...
species_map_data <- all_sources_species %>%
  ...
  select(-c(Study_Area, Description, Start_Deployment_date, End_Deployment_date,  Age, Habitat_type, Comments, Trap_type, Sample_ID, SPI_Project_name, Animal_ID, 
...
write.csv(marten_detect, file="Marten_records_March2025.csv")
getwd()
```
Map Fisher detections
...
library(tmap)
# Define custom colors
custom_colors <- c("Pekania pennanti" = "#00796B")  # Dark Teal
...
# Filter for Marten species, remove NA coordinates, and select necessary columns
species_map_data <- all_sources_species %>%
  filter(Species_scientific %in% c("Pekania pennanti")) %>%
  drop_na(Latitude, Longitude, Data_Sharing_Categorization) %>%  # Ensure no NAs in shape field
  mutate(
    Year_Study_Start = as.character(Year_Study_Start),  # Convert to character to avoid comma formatting
    Year_Study_End = as.character(Year_Study_End)  # Convert to character to avoid comma formatting
  ) %>%
  select(-c(Study_Area, Description, Start_Deployment_date, End_Deployment_date, Sex, Age, Habitat_type, Comments, Trap_type, Sample_ID, SPI_Project_name, Animal_ID, 
            Submission_File_link, Submission_File_name, Data_owner_names, 
...
tm_basemap("OpenStreetMap") +
  ...
  tm_dots(col = "Species", shape = "Data_Sharing_Categorization", 
          size = 0.6, palette = custom_colors, shape.palette = custom_shapes, 
          title.shape = "Data Permissions") +  # Shape legend title is working
  tm_layout(title = "Fisher Detections in British Columbia",
            legend.outside = TRUE)+ # Move legend outside for better visibility
    tm_view(set.zoom.limits = c(5, 7))  # Adjust zoom limits
